...
func TestStore_CodeExpiration(t *testing.T) {
	...
	// But in unit test we can't easily mock time.Now() unless we injected a clock.
	// For this simple project, we will use a very short duration and sleep.
...
func TestStore_Cleanup(t *testing.T) {
	...
	// Note: GetCode checks expiration internally, so we check existence via internal map if we want to be sure it wasn't deleted?
	// But public API is fine.
